...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        con = self.con(list[1].permute(0, 3, 1, 2))
        speed = self.first_conv(list[0].permute(0, 3, 1, 2))

        con = con * self.weight_con + self.bias_con
        speed = speed * self.weight_speed
        x = torch.add(speed, con)
        x = self.BlockList1(x.permute(0, 2, 3, 1))
        x = self.BlockList2(x)  # [B, N, F, T] -> [B, T, N, F]

        x = self.final_conv(x)[:, :, :, -1] # 16, 12, 131
        x,_ = self.lstm(x.permute(2,0,1)) # 131, 16, 64
