...
def evaluation(a, b):
    ...
    mape = np.divide(mae, a)
    mape = np.nan_to_num(mape)
    mape = np.mean(mape)
    return rmse, mae, 1 - F_norm, r2, var, mape


x_axe, batch_loss, batch_rmse, batch_pred = [], [], [], []
test_loss, test_rmse, test_mae, test_mape, test_acc, test_r2, test_var, test_pred = [], [], [], [], [], [], [], []
train_rmse_set = []
validation_rmse_set = []
epoch_set = []
...
for epoch in range(training_epoch):
    ...
    train_rmse_set.append(np.mean(batch_rmse))
    epoch_set.append(epoch)
    # Test completely at every epoch
    loss2, rmse2, test_output = sess.run([loss, error, y_pred],
                                         feed_dict={inputs: testX, labels: testY})
    test_label = np.reshape(testY, [-1, num_nodes])
    rmse, mae, acc, r2_score, var_score, mape = evaluation(test_label, test_output)
    validation_rmse_set.append(rmse * max_value)
...
d = {'epoch': epoch_set, 'training_rmse': train_rmse_set,  'validation_rmse': validation_rmse_set}
df = pd.DataFrame(data=d)
df.to_csv('result/T-GCN.csv', index=False)
############## visualization ###############
...
print('min_rmse:%r' % (np.mean(test_rmse)),
